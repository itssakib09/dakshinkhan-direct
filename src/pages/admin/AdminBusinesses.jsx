...
function DeleteConfirmModal({ business, onCancel, onConfirm, deleting }) {
  const [inputValue, setInputValue] = useState('')
  const storeName = business.storeSettings?.storeName || ''
  const matches = inputValue.trim() === storeName.trim() && storeName.trim().length > 0
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xl w-full max-w-sm p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          ...
          and cannot be undone. Type the store name below to confirm.
        </p>
        <input
          type="text"
          value={inputValue}
...
function AdminBusinesses() {
  ...
  async function loadBusinesses() {
    setLoading(true)
    const result = await getBusinesses({ pageSize: 20, lastDoc: null, searchTerm, statusFilter })
  ...
  async function loadMore() {
    setLoadingMore(true)
    const result = await getBusinesses({ pageSize: 20, lastDoc: lastVisibleDoc, searchTerm, statusFilter })
  ...
  async function handleToggleActive(business) {
    const currentStatus = business.storeSettings?.storeActive || false
    const newStatus = await toggleBusinessActive(business.id, currentStatus)
    setBusinesses(prev =>
      prev.map(b =>
        b.id === business.id
          ? { ...b, storeSettings: { ...b.storeSettings, storeActive: newStatus } }
          : b
      )
    )
  }

  async function handleToggleVerified(business) {
    const currentStatus = business.storeSettings?.verified || false
    const newStatus = await toggleBusinessVerified(business.id, currentStatus)
    setBusinesses(prev =>
      prev.map(b =>
        b.id === business.id
          ? { ...b, storeSettings: { ...b.storeSettings, verified: newStatus } }
          : b
      )
    )
  }
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true)
    const success = await deleteBusinessPermanently(deleteTarget.id)
    if (success) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5">
            Search matches from the start of the business name or phone number
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-10 text-center">
          <HiOfficeBuilding size={40} className="text-gray-300 dark:text-gray-600 mx-auto mb-3" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700 text-left">
                  <th className="px-4 py-3 font-bold text-gray-500 dark:text-gray-400">Store Name</th>
                  <th className="px-4 py-3 font-bold text-gray-500 dark:text-gray-400">Category</th>
              ...
              <tbody>
                <AnimatePresence>
                  {businesses.map(business => {
                    const storeName = business.storeSettings?.storeName || 'Unnamed Store'
                    const category = business.storeSettings?.businessType || '-'
                    const phone = business.storeSettings?.phone || business.phone || '-'
                    const isActive = business.storeSettings?.storeActive || false
                    const isVerified = business.storeSettings?.verified || false
                    return (
                      <motion.tr
                        key={business.id}
                        layout
                      ...
                      >
                        <td className="px-4 py-3 font-semibold text-gray-900 dark:text-white">{storeName}</td>
                        <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{category}</td>
                        <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{phone}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <ToggleSwitch checked={isActive} onChange={() => handleToggleActive(business)} activeColor="bg-green-500" />
                            <StatusBadge active={isActive} />
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <ToggleSwitch checked={isVerified} onChange={() => handleToggleVerified(business)} activeColor="bg-primary-600" />
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-black text-gray-900 dark:text-white">{storeName}</span>
                      <div className="flex items-center gap-3">
                        <a
                          href={`/store/${business.id}`}
                          target="_blank"
