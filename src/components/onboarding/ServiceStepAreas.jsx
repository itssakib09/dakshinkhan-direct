...
function ServiceStepAreas({ formData, updateFormData, onNext, onBack }) {
  ...
  const toggleArea = (area) => {
    ...
    if (area === ALL_AREAS_VALUE) {
      if (coverageAreas.includes(ALL_AREAS_VALUE)) {
        updateFormData({ coverageAreas: [] })
      } else {
        updateFormData({ coverageAreas: [ALL_AREAS_VALUE] })
      }
      return
    }

    const withoutAll = coverageAreas.filter(a => a !== ALL_AREAS_VALUE)

    if (withoutAll.includes(area)) {
      updateFormData({ coverageAreas: withoutAll.filter(a => a !== area) })
    } else {
      updateFormData({ coverageAreas: [...withoutAll, area] })
    }
  }
            ...
            <div>
              ...
              {formData.coverageAreas && formData.coverageAreas.length > 0 && (
                <p className="mt-3 text-sm text-primary-600 dark:text-primary-400 font-medium">
                  {formData.coverageAreas.length} area{formData.coverageAreas.length !== 1 ? 's' : ''} selected
                </p>
              )}
            </div>
          </>
...
export default ServiceStepAreas